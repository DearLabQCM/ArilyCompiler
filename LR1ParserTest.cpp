...
int main()
{
    vector<string>progs;
    progs.push_back("{ }");
    progs.push_back("{ while ( ID == NUM + ID * NUM - ID / NUM ) if ( ID >= NUM * NUM + ID ) then ID = NUM ; else ID = NUM + ID - ID * NUM / NUM ; }");
    progs.push_back(" { { { } } }");

    //Error
    progs.push_back(" {   ");
    progs.push_back(" }   ");
    progs.push_back("if ( ID <= NUM then ID = NUM  ID = NUM - ID }");
    progs.push_back("while ( ID == NUM if ( ID <= NUM then ID = NUM + ID * NUM ID = NUM - ID / NUM }");
...
string Generator(int step)
{
    ...
    while(step--)
    {
        ...
        if(generator%4 == 1)
        {
            ret+=Basics[0+(step+generator)%2];
        }
        else if(generator%4 == 2)
        {
            ret+=Basics[2+(step+generator)%2];
            ret+=Basics[0+(step+generator)%2];
            if(generator%2)ret+=Basics[2+2];
            else ret+=" ";
            ret+=Basics[0+(step+generator+1)%2];
        }
        else
        {
            ret+=Basics[5+(step+generator)%2];
            ret+=Basics[0+(step+generator)%2];
        }
    }
